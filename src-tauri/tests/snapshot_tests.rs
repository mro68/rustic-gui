use rustic_gui_lib::types::{SnapshotDto, RetentionPolicy};
...
async fn test_get_snapshot_not_found() {
    ...
    assert!(matches!(result, Err(RusticGuiError::RepositoryNotFound { .. }) | Err(RusticGuiError::SnapshotNotFound { .. })));
}
...
async fn test_delete_snapshot_not_found() {
    ...
    assert!(matches!(result, Err(RusticGuiError::RepositoryNotFound { .. }) | Err(RusticGuiError::Internal(_))));
}
