...
impl AppConfig {
    ...
    pub fn load() -> Result<Self> {
        ...
        ### PortableStore (AES-256-GCM)

        - Portable Builds nutzen **nicht** die oben beschriebenen Standardpfade. Stattdessen übernimmt `src-tauri/src/storage/portable.rs` das Lesen und Schreiben einer verschlüsselten `portable-config.dat` im gleichen Ordner wie das Binary.
        - Verschlüsselung: AES-256-GCM, Schlüssel abgeleitet aus einer Passphrase (`derive_key`-Hilfsfunktion). Niemals Klartext-Konfiguration speichern oder persistente Keys loggen.
        - Fallback: Wenn das Medium schreibgeschützt ist, wird automatisch auf ein temporäres Verzeichnis ausgewichen. In diesem Fall sendet der State ein `portable-store-fallback`-Event — stelle sicher, dass Frontend/Logs diesen Zustand kommunizieren.
        - Erweiterungen am Config-Schema müssen den PortableStore mit abdecken (serialize/deserialise, Migrationen).
